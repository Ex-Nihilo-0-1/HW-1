...
Contrary to the original specification of the program where `ID3.pruning()` has no `return` and updates the input `tree` directly, the same function in our submission returns the pruned tree based on a copy of `tree`. This is because we for the life of us couldn't figure out why we cannot use the function, which has a nested helper function in it, to update the tree directly. 
